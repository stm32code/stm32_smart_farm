...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	if(20 < n_heart_rate && n_heart_rate < 150 && ch_spo2_valid){
		memset(text, 0, sizeof(text));
		sprintf(text, "\"heart\":\"%d\",", n_heart_rate); 
		strcat(buf, text);
	}
	memset(text, 0, sizeof(text));
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp_v\":\"%.2f\",", Data_init.temperatuer_1); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"mode\":\"%d\",", Data_init.Flage);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"relay1\":\"%d\",",  (u8)relay1in); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"relay2\":\"%d\",", (u8)relay2in); 
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp_t\":\"%d\",",  threshold_value_init.temp_value); 
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi_t\":\"%d\"", threshold_value_init.humi_value); 
	strcat(buf, text);
...
unsigned char Mqtt_Pwd_Info(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"re_id\":\"%02X%02X%02X%02X\"", ucArray_ID [ 0 ],
                  ucArray_ID [ 1 ],
                  ucArray_ID [ 2 ],
                  ucArray_ID [ 3 ] ); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
